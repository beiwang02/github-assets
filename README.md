...
- `newest` 排序键及原 localStorage 设置不变；图片及引用界面称为“最近添加/最早添加”，分别使用原始 `createdAt` / `addedAt`；库条目仍按更新时间排序，修改不会重置历史加入时间。
- 上传、加入引用、编辑引用、图片/分组改名均在同一原子提交中记录实际变更时间；改名同步关联引用和库的更新时间。未变化的保存、重复加入或空删除不提交、不刷新时间；分组改名不触碰无关库。
- 概览各库封面独立取最近添加的 3 项，不受工作区排序影响；最近图片按创建时间取最新 6 项，概览库按更新时间排列。仅排序副本，JSON 原始数组顺序、公开字段及编辑/删除原索引保持不变。
