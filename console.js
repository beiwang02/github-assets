const S = {
  ...
  groups:[], assets:[], libraries:[], repos:[], selected:new Set(), selectedIcons:new Set(), group:'', assetQuery:'', libraryQuery:'', iconQuery:'', assetSort:localStorage.getItem('gh-assets-sort')||'newest', librarySort:localStorage.getItem('gh-libraries-sort')||'updated-desc', iconSort:localStorage.getItem('gh-icons-sort')||'newest', activity:[], uploadDraft:null, modalConfirm:null
};
...
function sortedAssetsC(items,mode=S.assetSort){return [...items].sort((a,b)=>mode==='name-asc'?collatorC.compare(a.name,b.name):mode==='name-desc'?collatorC.compare(b.name,a.name):compareTimedC(a,b,mode==='updated-desc'?'assetLatest':'createdAt',mode==='oldest'?1:-1));}
...
function sortedIconsC(lib,items,mode=S.iconSort){return [...items].sort((a,b)=>mode==='name-asc'?collatorC.compare(a.name,b.name):mode==='name-desc'?collatorC.compare(b.name,a.name):compareTimedC(a,b,mode==='updated-desc'?'latest':'addedAt',mode==='oldest'?1:-1));}
...
function overviewView() {
  ...
  return `<div class="hero"><div class="hero-content"><span class="mini-label">GITHUB RESOURCE HUB</span><h2>把每一张图片，变成可复用的资源。</h2><p>集中管理 GitHub 图床、图片分组与 JSON 库，复制一条直链，就能在任何项目里使用。</p><div class="hero-actions"><button class="btn btn-primary" data-action="upload">＋ 上传图片</button><button class="btn" data-action="new-library">▦ 新建 JSON 库</button><button class="btn" data-action="create-repo">＋ 创建我的图床仓库</button></div></div></div><div class="stat-grid">${statC('▧','图片资源',S.connected?S.assets.length:'0',S.connected?'已读取':'未连接',S.connected?`分布在 ${S.groups.length} 个分组`:'连接仓库后显示真实数据')}${statC('▦','JSON 库',S.connected?String(S.libraries.length):'0',S.connected?'已读取':'未连接',S.connected?`共 ${refs} 个图片引用`:'连接仓库后显示真实数据')}${statC('✓','仓库状态',S.connected?'正常':'未连接',S.connected?'在线':'等待连接',S.connected?'最后读取：刚刚':'请先连接 GitHub')}${statC('↗','本次同步',S.connected?'已读取':'0',S.connected?'正常':'未连接',S.connected?'数据来自 GitHub':'连接仓库后显示同步记录')}</div><div class="dashboard-columns"><section><div class="section-row"><h3>JSON 库</h3><button class="text-link" data-view="libraries">查看全部 →</button></div><div class="card library-card"><div class="library-card-header"><div class="library-title"><div class="library-logo" style="background:linear-gradient(135deg,#6672ff,#8d64e8)">▦</div><div><b>我的 JSON 库</b><small>GitHub 上的 JSON 引用集合</small></div></div></div>${S.libraries.length?sortedLibrariesC(S.libraries,'updated-desc').slice(0,5).map(lib=>lib.count?`<div class="library-list-row" data-action="open-library" data-id="${escC(lib.id)}">${coverStack(lib)}<div class="list-info"><b>${escC(lib.name)}</b><small>${escC(lib.description||lib.file)}</small></div><div class="list-meta"><strong>${lib.count} 个</strong><span>已读取</span></div></div>`:`<div class="library-empty-row" data-action="open-library" data-id="${escC(lib.id)}"><div class="empty-icon">▦</div><div><b>暂无图片引用</b><small>${escC(lib.name)} · 进入库内添加图片</small></div></div>`).join(''):emptyC('▦','尚未读取 JSON 库',S.connectionError||'登录后创建你自己的图床仓库。',S.isAdmin?'settings':'create-repo',S.isAdmin?'打开仓库设置':'创建我的图床仓库')}</div></section><section><div class="section-row"><h3>最近动态</h3><button class="text-link" data-view="activity">全部记录 →</button></div><div class="card activity-card">${activityView(4)}</div></section></div>`;
}
function librariesView() {
  ...
  return `<div class="page-heading json-workspace-heading"><div><p>选择当前库后，在同一工作区维护库信息与图片引用。</p></div>${S.connected?`<div class="heading-actions"><button class="btn btn-primary" data-action="new-library">＋ 新建 JSON 库</button></div>`:''}</div><section class="card json-workspace"><div class="json-workspace-top"><div class="library-current-info"><span class="library-logo" style="background:${lib.gradient||'linear-gradient(135deg,#6672ff,#7b8af1)'}">${escC(lib.name.slice(0,1))}</span><span class="library-current-copy"><b>${escC(lib.name)}</b><em>${lib.count} 个图片引用 · ${escC(lib.file)}</em></span></div><button type="button" class="library-switch-trigger" data-action="open-library-picker" aria-haspopup="dialog" aria-expanded="false">切换库</button></div>${lib.description?`<p class="json-library-summary">${escC(lib.description)}</p>`:''}<div class="library-outside-actions"><div><button class="btn" data-action="edit-library" data-id="${escC(lib.id)}">编辑库信息</button><button class="btn" data-action="copy" data-copy="${escC(rawLibrary(lib))}">复制 JSON 直链</button></div></div></section><div class="toolbar json-reference-toolbar"><label class="inner-search"><span>⌕</span><input data-bind="icon-search" value="${escC(S.iconQuery)}" placeholder="搜索当前库的图片名称或直链…"></label>${sortSelectC('icons',S.iconSort,[['newest','最近添加'],['oldest','最早添加'],['name-asc','名称 A-Z'],['name-desc','名称 Z-A']])}<span class="toolbar-spacer"></span><button class="btn btn-sm" data-action="refresh">↻ 刷新</button></div><section class="json-references"><div class="section-row"><h3>图片引用</h3><div class="section-actions" data-role="icons-selection-toolbar">${S.selectedIcons.size?`<span class="select-counter">已选择 ${S.selectedIcons.size} 张</span>`:''}<button class="btn btn-sm" data-action="select-all-icons">全选</button>${S.selectedIcons.size?`<button class="btn btn-sm btn-danger" data-action="delete-selected-icons">删除</button>`:''}</div></div><div class="json-reference-list">${icons.length?icons.map(icon=>`<article class="json-reference-row ${S.selectedIcons.has(icon.index)?'selected':''}" data-action="icon-open" data-index="${icon.index}"><button class="reference-select" data-action="select-icon" data-index="${icon.index}" aria-label="${S.selectedIcons.has(icon.index)?'取消选择':'选择'} ${escC(icon.name)}" title="${S.selectedIcons.has(icon.index)?'取消选择':'选择'} ${escC(icon.name)}" aria-pressed="${S.selectedIcons.has(icon.index)}">${S.selectedIcons.has(icon.index)?'✓':''}</button><span class="json-reference-thumb"><img src="${escC(imageURLC(icon))}" alt="${escC(icon.name)}" loading="lazy"></span><div class="json-reference-copy"><b>${escC(icon.name)}</b></div><div class="json-reference-actions"><button class="icon-btn copy-control copy-control-icon" aria-label="复制 ${escC(icon.name)} 直链" title="复制直链" data-action="copy" data-copy="${escC(icon.url)}">⧉</button><button class="icon-btn" aria-label="删除 ${escC(icon.name)} 引用" title="删除引用" data-action="delete-icon" data-index="${icon.index}">×</button></div></article>`).join(''):(q?emptyC('⌕','没有匹配的图片引用','请修改或清空搜索条件。'):emptyC('▦','这个 JSON 还没有图片引用','可以上传图片后加入 JSON 库。','new-icon','＋ 添加图片'))}</div></section>`;
}
...
async function bootAuth(retried=false) { if(location.protocol!=='http:'&&location.protocol!=='https:'){ renderC(); return; } try { const r=await fetch('/api/auth/me',{credentials:'include'}), data=await r.json(); S.auth=data.user||null; S.csrf=data.csrf||''; S.oauthEnabled=Boolean(data.oauthEnabled); S.tokenLoginEnabled=Boolean(data.tokenLoginEnabled); S.adminConfigured=Boolean(data.adminConfigured); S.isAdmin=Boolean(data.isAdmin); S.policyConfigured=Boolean(data.policyConfigured); if(!S.auth&&!retried&&S.tokenLoginEnabled&&rememberedToken()&&await silentTokenLogin()) return bootAuth(true); if(S.auth) { await loadAdminPolicy(); renderC(); if(sessionStorage.getItem('gh-login-success')==='1'){ sessionStorage.removeItem('gh-login-success'); notify('已登录'); } await autoSelectRepository(); } } catch { S.auth=null; } renderC(); }
...
文件：${lib.file}`,async()=>{await currentClient().deleteLibrary(lib.file);closeC();await refreshCommittedC();notify('JSON 文件已删除');});return;}
  ...
  if(action==='new-group-from-upload'){if(!S.connected)return notify('请先连接你的仓库','error');groupModal(true);return;}
...
function overviewQuickAssets() {
  ...
  return `<section class="overview-assets-section"><div class="section-row"><h3>最近图片资源</h3><button class="text-link" data-view="assets">查看全部 →</button></div><div class="card quick-assets-grid">${sortedAssetsC(S.assets,'newest').slice(0,6).map(item=>`<div class="quick-asset" data-action="asset-open" data-id="${escC(item.id)}"><div class="quick-asset-image"><img src="${escC(imageURLC(item))}" alt="${escC(item.name)}" loading="lazy"></div><div class="quick-asset-name-row"><b title="${escC(item.name)}" aria-label="${escC(item.name)}">${escC(item.name)}</b><button class="quick-copy copy-control copy-control-icon" data-action="copy" data-copy="${escC(item.url)}" title="复制直链" aria-label="复制 ${escC(item.name)} 直链">⧉</button></div><small>${escC(item.group||'根目录')}</small></div>`).join('')}</div></section>`;
}
