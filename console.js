...
function librariesView() {
  ...
  return `<div class="page-heading json-workspace-heading"><div><p>选择当前库后，在同一工作区维护库信息与图片引用。</p></div><div class="heading-actions"><button class="btn btn-primary" data-action="new-library">＋ 新建 JSON 库</button></div></div><section class="card json-workspace"><div class="json-workspace-top"><button class="library-picker" data-action="open-library-picker" aria-haspopup="dialog" aria-label="切换当前 JSON 库"><span class="library-logo" style="background:${lib.gradient||'linear-gradient(135deg,#6672ff,#7b8af1)'}">${escC(lib.name.slice(0,1))}</span><span class="library-picker-copy"><small>当前 JSON 库</small><b>${escC(lib.name)}</b><em>${lib.count} 个图片引用 · ${escC(lib.file)}</em></span><span class="library-picker-arrow">⌄</span></button><div class="json-workspace-actions"><button class="btn btn-sm" data-action="copy" data-copy="${escC(rawLibrary(lib))}">⧉ 复制直链</button><button class="btn btn-sm" data-action="edit-library" data-id="${escC(lib.id)}">编辑</button></div></div>${lib.description?`<div class="json-library-summary"><div><p>${escC(lib.description)}</p></div></div>`:''}</section><div class="toolbar json-reference-toolbar"><label class="inner-search"><span>⌕</span><input data-bind="icon-search" value="${escC(S.iconQuery)}" placeholder="搜索当前库的图片名称或直链…"></label><span class="toolbar-spacer"></span><button class="btn btn-sm" data-action="refresh">↻ 刷新</button></div><section class="json-references"><div class="section-row"><h3>图片引用</h3><div class="section-actions" data-role="icons-selection-toolbar"><span class="select-counter">${icons.length} 个结果</span><button class="btn btn-sm" data-action="select-all-icons">全选</button>${S.selectedIcons.size?`<button class="btn btn-sm btn-danger" data-action="delete-selected-icons">删除</button>`:''}</div></div><div class="json-reference-list">${icons.length?icons.map(icon=>`<article class="json-reference-row ${S.selectedIcons.has(icon.index)?'selected':''}"><button class="reference-select" data-action="select-icon" data-index="${icon.index}" aria-label="${S.selectedIcons.has(icon.index)?'取消选择':'选择'} ${escC(icon.name)}" title="${S.selectedIcons.has(icon.index)?'取消选择':'选择'} ${escC(icon.name)}" aria-pressed="${S.selectedIcons.has(icon.index)}">${S.selectedIcons.has(icon.index)?'✓':''}</button><img src="${escC(icon.url)}" alt="${escC(icon.name)}" loading="lazy"><div class="json-reference-copy"><b>${escC(icon.name)}</b></div><span class="json-reference-status"><i class="status-dot"></i>已同步</span><div class="json-reference-actions"><button class="icon-btn" aria-label="复制 ${escC(icon.name)} 直链" title="复制直链" data-action="copy" data-copy="${escC(icon.url)}">⧉</button><button class="icon-btn" aria-label="编辑 ${escC(icon.name)} 引用" title="编辑引用" data-action="edit-icon" data-index="${icon.index}">✎</button><button class="icon-btn" aria-label="删除 ${escC(icon.name)} 引用" title="删除引用" data-action="delete-icon" data-index="${icon.index}">×</button></div></article>`).join(''):emptyC('▦','这个 JSON 还没有图片引用','可以添加已有 Raw 直链，或上传图片后加入当前 JSON。','new-icon','＋ 添加图片')}</div></section>`;
}
...
function renderAssetsToolbar() {
  ...
  if(selected){ area.insertAdjacentHTML('beforeend',`<span class="select-counter">已选择 ${selected} 张</span><button class="btn btn-sm" data-action="bulk-library">加入 JSON</button><button type="button" class="btn btn-sm btn-danger" data-action="bulk-delete">删除</button>`); }
}
...
function renderIconsToolbar() {
  ...
  area.innerHTML=`<span class="select-counter">${icons.length} 个结果</span><button class="btn btn-sm" data-action="select-all-icons">${allSelected?'取消全选':'全选'}</button>${S.selectedIcons.size?'<button class="btn btn-sm btn-danger" data-action="delete-selected-icons">删除</button>':''}`;
}
...
async function autoSelectRepository() {
  ...
  try {
    ...
    if (!found) { S.connectionError = candidates.length ? `发现 ${candidates.length} 个符合图床结构的仓库，请选择或创建新仓库。` : '没有自动选择仓库，请创建你自己的图床仓库。'; renderC(); return; }
