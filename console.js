...
function relativeTimeC(value){
  ...
  const day=String(then.getDate()).padStart(2,'0'),month=String(then.getMonth()+1).padStart(2,'0');
  const clock=`${String(then.getHours()).padStart(2,'0')}:${String(then.getMinutes()).padStart(2,'0')}`;
  const yesterday=new Date(now);yesterday.setDate(now.getDate()-1);
  if(then.toDateString()===yesterday.toDateString())return `昨天 ${clock}`;
  return sameYear?`${month}-${day} ${clock}`:`${then.getFullYear()}-${month}-${day}`;
}
function activityView(limit=20) { return S.activity.length?S.activity.slice(0,limit).map(a=>`<div class="activity-item"><div class="activity-line"><i class="activity-dot"></i></div><div class="activity-copy"><b>${escC(a.title)}</b><small>${escC(a.detail)} · ${escC(relativeTimeC(a.time))}</small></div></div>`).join(''):emptyC('◷','暂无同步记录','读取仓库或提交操作后会显示在这里。'); }
function overviewView() {
  const refs=S.libraries.reduce((n,x)=>n+(x.count||0),0);
  return `<div class="hero"><div class="hero-content"><span class="mini-label">GITHUB RESOURCE HUB</span><h2>把每一张图片，变成可复用的资源。</h2><p>管理图片分组与 JSON 库，随时复制直链。</p><div class="hero-actions"><button class="btn btn-primary" data-action="upload">＋ 上传图片</button></div></div></div><div class="stat-grid overview-stat-grid">${statC('▧','图片资源',S.connected?S.assets.length:'0',S.connected?'已读取':'未连接',S.connected?`分布在 ${S.groups.length} 个分组`:'连接仓库后显示真实数据')}${statC('▦','JSON 库',S.connected?String(S.libraries.length):'0',S.connected?'已读取':'未连接',S.connected?`共 ${refs} 个图片引用`:'连接仓库后显示真实数据')}</div><p class="overview-connection">${S.connected?`仓库已读取 · ${escC(S.repo.owner)}/${escC(S.repo.repo)}${S.syncError?` · ${escC(S.syncError)}`:''}`:(S.connectionError?`仓库未读取 · ${escC(S.connectionError)}`:'仓库未连接 · 可前往仓库设置连接')}</p><div class="dashboard-columns"><section><div class="section-row"><h3>JSON 库</h3><button class="text-link" data-view="libraries">查看全部 →</button></div><div class="card library-card">${S.libraries.length?sortedLibrariesC(S.libraries,'updated-desc').slice(0,5).map(lib=>lib.count?`<div class="library-list-row" data-action="open-library" data-id="${escC(lib.id)}">${coverStack(lib)}<div class="list-info"><b>${escC(lib.name)}</b><small>${escC(lib.description||lib.file)}</small></div><div class="list-meta"><strong>${lib.count} 个</strong><span>已读取</span></div></div>`:`<div class="library-empty-row" data-action="open-library" data-id="${escC(lib.id)}"><div class="empty-icon">▦</div><div><b>暂无图片引用</b><small>${escC(lib.name)} · 进入库内添加图片</small></div></div>`).join(''):emptyC('▦',S.connected?'还没有 JSON 库':'尚未连接仓库',S.connected?'在当前仓库中新建 JSON 库，即可管理图片引用。':(S.connectionError||'前往仓库设置连接已有仓库；需要时也可在那里新建。'),S.connected?'new-library':'settings',S.connected?'新建 JSON 库':'前往仓库设置')}</div></section><section class="overview-activity"><div class="section-row"><h3>最近动态</h3><button class="text-link" data-view="activity">全部记录 →</button></div><div class="overview-activity-list">${S.activity.length?activityView(3):'<p class="overview-activity-empty">暂无同步记录</p>'}</div></section></div>`;
}
...
overviewView = function() {
  ...
  const marker = '<div class="dashboard-columns">';
  const pos = base.indexOf(marker);
  return pos >= 0 ? `${base.slice(0,pos)}${overviewQuickAssets()}${base.slice(pos)}` : `${base}${overviewQuickAssets()}`;
};
