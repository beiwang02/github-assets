...
function iconModal(icon) { openC(`<div class="modal-head"><div><h2>${escC(icon.name)}</h2><p>JSON 图片引用详情</p></div><button class="modal-close" data-action="close-modal">×</button></div><div class="modal-body"><div class="json-icon-detail-preview"><img src="${escC(icon.url)}" alt="${escC(icon.name)}"></div><div class="modal-field"><label>完整 Raw 直链</label><input value="${escC(icon.url)}" readonly></div></div><div class="modal-actions"><button class="btn btn-danger" data-action="delete-icon" data-index="${icon.index}">删除引用</button><button class="btn" data-action="copy" data-copy="${escC(icon.url)}">⧉ 复制直链</button><button class="btn btn-primary" data-action="close-modal">完成</button></div>`); }
