const S = {
  ...
  groups:[], assets:[], libraries:[], repos:[], selected:new Set(), group:'', assetQuery:'', libraryQuery:'', iconQuery:'', activity:[]
};
...
function detailView() {
  ...
  return `<div class="page-heading"><div><button class="text-link" data-view="libraries">← 返回 JSON 库</button><h2 style="margin-top:10px">${escC(lib.name)}</h2><p>${escC(lib.description||'')} · ${escC(lib.file)}</p></div><div class="heading-actions"><button class="btn" data-action="copy" data-copy="${escC(rawLibrary(lib))}">⧉ 复制 JSON 直链</button><button class="btn" data-action="edit-library" data-id="${escC(lib.id)}">编辑库信息</button><button class="btn btn-primary" data-action="new-icon">＋ 添加图片</button></div></div><div class="card detail-header"><div class="library-logo" style="background:${lib.gradient||'linear-gradient(135deg,#6672ff,#7b8af1)'}">${escC(lib.name.slice(0,1))}</div><div><h2>${escC(lib.name)}</h2><p>文件：${escC(lib.file)} · ${lib.count} 个图片引用</p></div><div class="detail-actions"><button class="icon-btn" data-action="edit-library" data-id="${escC(lib.id)}">✎</button><button class="icon-btn" data-action="copy" data-copy="${escC(rawLibrary(lib))}">⧉</button></div></div><div class="toolbar"><label class="inner-search"><span>⌕</span><input data-bind="icon-search" value="${escC(S.iconQuery)}" placeholder="搜索名称或直链…"></label><span class="toolbar-spacer"></span><button class="btn btn-sm" data-action="refresh">↻ 刷新</button></div><div class="library-detail-grid">${icons.length?icons.map(icon=>`<article class="card library-detail-card"><div class="library-detail-image"><img src="${escC(icon.url)}" alt="${escC(icon.name)}" loading="lazy"><span class="library-detail-status"><span class="status-dot"></span>已同步</span></div><div class="library-detail-copy"><h3>${escC(icon.name)}</h3><p>JSON 引用</p><span class="library-detail-url" title="${escC(icon.url)}">${escC(icon.url)}</span></div><div class="library-detail-actions"><button class="icon-btn" data-action="copy" data-copy="${escC(icon.url)}" title="复制直链" aria-label="复制直链">⧉</button><button class="icon-btn" data-action="edit-icon" data-index="${icon.index}" title="编辑引用" aria-label="编辑引用">✎</button><button class="icon-btn" data-action="delete-icon" data-index="${icon.index}" title="删除引用" aria-label="删除引用">×</button></div></article>`).join(''):emptyC('▦','这个 JSON 还没有图片引用','可以从图片资源中上传并加入，或添加一个已有 Raw 直链。')}</div>`;
}
...
function tokenGuideModal() { openC(`<div class="modal-head"><div><h2>经典 Token 创建教程</h2><p>仅用于测试登录 GitHub 图床。</p></div><button class="modal-close" data-action="close-modal">×</button></div><div class="modal-body token-guide-body"><ol><li>打开 GitHub → Settings → Developer settings → Personal access tokens → Tokens (classic)。</li><li>点击 Generate new token (classic)，设置有效期。</li><li>权限列表只勾选 <b>repo → public_repo</b>。</li><li>其他权限不要勾选，生成后复制 Token 粘贴到登录框。</li></ol></div>`); }
...
async function formSubmit(e) {
  ...
  try {
    ...
    if(form.id==='libraryForm') { const data=new FormData(form), name=String(data.get('name')).trim(), description=String(data.get('description')).trim(), path=String(data.get('path')).trim(); if(!name)return notify('请填写 JSON 库名称','error'); const old=S.libraries.find(x=>x.id===S.editingLibrary); if(old) await currentClient().saveLibrary(old.file,path,name,description); else await currentClient().createLibrary(path,name,description); closeC(); await refreshRepo(); notify('JSON 库已提交'); return; }
    if(form.id==='iconForm') { const data=new FormData(form), lib=S.libraries.find(x=>x.id===S.selectedLibrary); await currentClient().saveIcon(lib.file,Number(data.get('index')),String(data.get('name')),String(data.get('url')),lib.sha); closeC(); await refreshRepo(); S.view='library-detail'; renderC(); notify('图片引用已更新'); return; }
  ...
  } catch(error) { const submit=form?.querySelector?.('[type="submit"]'); if(submit){submit.disabled=false;submit.textContent='验证并登录';} notify(error.message||'操作失败','error'); }
}
...
async function autoSelectRepository() {
  ...
  try {
    ...
    S.connected = true; S.repo.assetsPath = data.root; S.groups=data.groups; S.group=S.group&&data.groups.some(g=>g.name===S.group)?S.group:(data.groups[0]?.name||''); S.assets=data.assets; S.libraries=data.libraries.map((x,i)=>({...x,gradient:['linear-gradient(135deg,#7580ff,#8c64e9)','linear-gradient(135deg,#ffb26d,#eb7574)','linear-gradient(135deg,#43cec4,#5aa8e8)'][i%3]}));
...
async function bootAuth() { if(location.protocol!=='http:'&&location.protocol!=='https:'){ renderC(); return; } try { const r=await fetch('/api/auth/me',{credentials:'include'}), data=await r.json(); S.auth=data.user||null; S.csrf=data.csrf||''; S.oauthEnabled=Boolean(data.oauthEnabled); S.tokenLoginEnabled=Boolean(data.tokenLoginEnabled); S.adminConfigured=Boolean(data.adminConfigured); S.isAdmin=Boolean(data.isAdmin); S.policyConfigured=Boolean(data.policyConfigured); S.restoreRepo=data.restoreRepo||''; if(S.auth) { await loadAdminPolicy(); renderC(); if(sessionStorage.getItem('gh-login-success')==='1'){ sessionStorage.removeItem('gh-login-success'); notify('已登录'); } await autoSelectRepository(); } } catch { S.auth=null; } renderC(); }
...
document.addEventListener('click', async e => {
  ...
  if(action==='select-all'){const q=S.assetQuery.trim().toLowerCase(), list=S.assets.filter(x=>(!S.group||x.group===S.group)&&(!q||x.name.toLowerCase().includes(q)||x.url.toLowerCase().includes(q))); if(list.length&&list.every(x=>S.selected.has(x.id)))list.forEach(x=>S.selected.delete(x.id));else list.forEach(x=>S.selected.add(x.id));renderC();return;}
  ...
  if(action==='delete-icon'){const lib=S.libraries.find(x=>x.id===S.selectedLibrary), index=Number(target.dataset.index);if(!lib||!confirm(`从 ${lib.name} 中移除这个图片引用？图片文件不会删除。`))return;try{await currentClient().removeIcons(lib.file,[index],lib.sha);await refreshRepo();S.view='library-detail';renderC();notify('图片引用已移除');}catch(err){notify(err.message,'error');}return;}
