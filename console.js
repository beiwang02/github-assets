...
function coverStack(lib) { const list=(lib.icons||[]).slice(0,4); return `<div class="cover-stack" aria-hidden="true">${list.length?list.map(x=>`<span class="cover-thumb"><img src="${escC(x.url)}" alt=""></span>`).join(''):'<span class="cover-empty">▦</span>'}</div>`; }
...
document.addEventListener('click', async e => {
  ...
  if(action==='use-repo'){S.repo={owner:target.dataset.owner,repo:target.dataset.repo,branch:target.dataset.branch||'main',assetsPath:target.dataset.assetsPath||'assets'};localStorage.setItem('gh-image-repo',JSON.stringify(S.repo));await readRepo({elements:{owner:{value:S.repo.owner},repo:{value:S.repo.repo},branch:{value:S.repo.branch},assetsPath:{value:S.repo.assetsPath}}});return;}
