...
const interactiveTapSelector='button,.btn,.icon-btn,.text-link,.nav-item,.repo-switcher,.repo-quick,.library-picker,.library-picker-option,.library-picker-create,.group-pill,.quick-asset,.asset-card,.json-reference-row,.asset-select,.asset-copy,.quick-copy,.modal-close,.top-icon,.project-link,.token-eye,.token-guide-button,.sort-trigger,.sort-option,.drop-zone';
document.addEventListener('pointerdown',e=>{const control=e.target.closest(interactiveTapSelector);if(!control||control.disabled||control.matches('.btn-danger,.library-picker-delete,[data-action="delete-asset"],[data-action="delete-icon"],[data-action="delete-library"],[data-action="delete-repo"],[data-action="bulk-delete"],[data-action="delete-selected-icons"],[data-action="confirm-delete-group"]'))return;control.classList.remove('tap-flash');void control.offsetWidth;control.classList.add('tap-flash');clearTimeout(control._tapFlashTimer);control._tapFlashTimer=setTimeout(()=>control.classList.remove('tap-flash'),320);},true);
