...
function libraryPickerModal() { const libraries=sortedLibrariesC(S.libraries); openC(`<div class="modal-head"><div><h2>切换 JSON 库</h2><p>选择已有库，或新建 JSON 库。</p></div><button class="modal-close" data-action="close-modal">×</button></div><div class="modal-body library-picker-modal"><div class="library-picker-list">${libraries.map(lib=>`<button class="library-picker-option ${lib.id===S.selectedLibrary?'selected':''}" data-action="select-library" data-id="${escC(lib.id)}"><span class="library-logo" style="background:${lib.gradient||'linear-gradient(135deg,#6672ff,#7b8af1)'}">${escC(lib.name.slice(0,1))}</span><span><b>${escC(lib.name)}</b><small>${lib.count} 个图片引用 · ${escC(lib.file)}</small></span>${lib.id===S.selectedLibrary?'<em>当前</em>':''}</button>`).join('')}</div><button class="library-picker-create" data-action="new-library">＋ 新建 JSON 库</button></div>`); }
