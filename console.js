...
function compareTimedC(a,b,field,direction){const x=timeC(a[field]),y=timeC(b[field]);if(x===null&&y===null)return 0;if(x===null)return 1;if(y===null)return -1;return direction*(x-y);}
...
function libraryPickerModal(anchor=document.querySelector('.library-switch-trigger')) {
 if(!anchor)return;
 let panel=anchor.parentElement.querySelector('.library-switch-panel');
 if(panel){const wasOpen=anchor.getAttribute('aria-expanded')==='true';window.AnchoredMenu.close(true);panel.remove();if(wasOpen)return;}
 panel=document.createElement('div');panel.className='library-switch-panel';panel.setAttribute('role','dialog');panel.setAttribute('aria-label','切换 JSON 库');
 panel.innerHTML=`<div class="library-switch-title"><span>切换 JSON 库</span></div><div class="library-switch-list">${sortedLibrariesC(S.libraries).map(lib=>`<div class="library-switch-row"><button class="library-switch-option" data-action="select-library" data-id="${escC(lib.id)}" aria-checked="${lib.id===S.selectedLibrary}"><span class="library-switch-check" ${lib.id===S.selectedLibrary?'role="img" aria-label="当前库"':'aria-hidden="true"'}>${lib.id===S.selectedLibrary?'✓':''}</span><span class="library-switch-copy"><b>${escC(lib.name)}</b><small>${lib.count} 个图片引用 · ${escC(lib.file)}</small></span></button><button type="button" class="library-switch-delete ui-button" data-ui="secondary" data-action="delete-library" data-id="${escC(lib.id)}" aria-label="删除JSON库${escC(lib.name)}">删除</button></div>`).join('')}</div><button class="library-picker-create" data-action="new-library">＋ 新建 JSON 库</button>`;
 panel.addEventListener('click',e=>{if(e.target.closest('[data-action]'))window.AnchoredMenu.close();});
 anchor.parentElement.append(panel);window.AnchoredMenu.open(anchor,panel);
}
...
function assetModal(item) { openC(`<div class="modal-head"><div><h2>${escC(item.name)}</h2><p>${escC(item.group||'根目录')} 分组 · ${escC(item.ext)} 图片资源</p></div><button class="modal-close" data-action="close-modal">×</button></div><div class="modal-body"><div class="asset-detail-preview"><img src="${escC(item.url)}" alt="${escC(item.name)}"></div><div class="detail-readonly"><b>GitHub Raw 直链</b><p class="detail-url">${escC(item.url)}</p></div><div class="detail-readonly"><b>仓库路径</b><p>${escC(item.path)}</p></div></div><div class="modal-actions asset-detail-actions"><button class="btn" data-action="copy" data-copy="${escC(item.url)}">⧉ 复制直链</button><button class="btn btn-primary" data-action="asset-library" data-id="${escC(item.id)}">加入 JSON 库</button><button class="btn" data-action="rename-asset" data-id="${escC(item.id)}">改名并同步引用</button><button class="btn btn-danger" data-action="delete-asset" data-id="${escC(item.id)}">删除图片</button></div>`); }
function iconModal(icon) { openC(`<div class="modal-head"><div><h2>${escC(icon.name)}</h2><p>JSON 图片引用详情</p></div><button class="modal-close" data-action="close-modal">×</button></div><div class="modal-body"><div class="json-icon-detail-preview"><img src="${escC(icon.url)}" alt="${escC(icon.name)}"></div><div class="detail-readonly"><b>GitHub Raw 直链</b><p class="detail-url">${escC(icon.url)}</p></div></div><div class="modal-actions icon-detail-actions"><button class="btn" data-action="copy" data-copy="${escC(icon.url)}">⧉ 复制直链</button><button class="btn btn-danger" data-action="delete-icon" data-index="${icon.index}">删除引用</button></div>`); }
...
document.addEventListener('click', async e => {
  ...
  if(action==='open-library-picker'){libraryPickerModal(target);return;}
...
function overviewQuickAssets() {
  ...
  return `<section class="overview-assets-section"><div class="section-row"><h3>最近图片资源</h3><button class="text-link" data-view="assets">查看全部 →</button></div><div class="card quick-assets-grid">${sortedAssetsC(S.assets,'newest').slice(0,6).map(item=>`<div class="quick-asset" data-action="asset-open" data-id="${escC(item.id)}"><div class="quick-asset-image"><img src="${escC(item.url)}" alt="${escC(item.name)}" loading="lazy"></div><div class="quick-asset-name-row"><b title="${escC(item.name)}" aria-label="${escC(item.name)}">${escC(item.name)}</b><button class="quick-copy" data-action="copy" data-copy="${escC(item.url)}" title="复制直链" aria-label="复制 ${escC(item.name)} 直链">⧉</button></div><small>${escC(item.group||'根目录')}</small></div>`).join('')}</div></section>`;
}
