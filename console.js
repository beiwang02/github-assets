...
async function formSubmit(e) {
  ...
  try {
    ...
    if(form.id==='uploadForm') { const group=String(form.elements.group.value||'').trim(), file=form.uploadFile||form.elements.file.files[0]; if(!group)return notify('当前没有图片分组，请先到图片资源页面新建分组。','error'); if(!file)throw new Error('请先选择一张图片。'); const result=await currentClient().upload(file,form.elements.name.value,group,form.elements.library.value); closeC(); await refreshRepo(); notify(`已上传“${result.name}”，并提交到 GitHub`); return; }
