...
function libraryPickerModal(anchor=document.querySelector('.library-picker')) {
 if(!anchor)return;
 ...
 panel.innerHTML=`<div class="library-switch-title">切换 JSON 库</div><div class="library-switch-list">${sortedLibrariesC(S.libraries).map(lib=>`<div class="library-switch-row"><button class="library-switch-option" data-action="select-library" data-id="${escC(lib.id)}" aria-checked="${lib.id===S.selectedLibrary}"><span class="library-logo">${escC(lib.name.slice(0,1))}</span><span class="library-switch-copy"><b>${escC(lib.name)}</b><small>${lib.count} 个图片引用 · ${escC(lib.file)}</small></span>${lib.id===S.selectedLibrary?'<em aria-label="当前库">✓</em>':''}</button>${lib.id===S.selectedLibrary?`<button class="library-switch-delete" data-action="delete-library" data-id="${escC(lib.id)}" aria-label="删除当前库 ${escC(lib.name)}">删除</button>`:''}</div>`).join('')}</div><button class="library-picker-create" data-action="new-library">＋ 新建 JSON 库</button>`;
...
function enhanceControlsC(){
  ...
  document.querySelectorAll('.library-list-row[data-action],.library-empty-row[data-action]').forEach(el=>{el.tabIndex=0;el.setAttribute('role','button');el.setAttribute('aria-haspopup','dialog');});
}
