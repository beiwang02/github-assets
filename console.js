...
function groupModal(fromUpload=false) { const draft=fromUpload?captureUploadDraft():null; nameModal('groupCreate','新建图片分组','','分组名称留空时请填写后保存；分组会作为目录创建在 GitHub 图床仓库中。'); S.uploadDraft=draft; }
...
function libraryModal(edit=false) { S.editingLibrary=edit?S.selectedLibrary:null; const lib=edit?S.libraries.find(x=>x.id===S.selectedLibrary):null; openC(`<div class="modal-head"><div><h2>${edit?'编辑 JSON 库':'新建 JSON 库'}</h2><p>${edit?'修改名称、说明或移动 JSON 文件。':'创建一个带 icons 数组的 JSON 文件；文件名填写时不需要输入 .json，系统会自动补全。'}</p></div><button class="modal-close" data-action="close-modal">×</button></div><form id="libraryForm"><div class="modal-body"><div class="modal-field"><label>JSON 库名称</label><input name="name" value="${escC(lib?.name||'')}" placeholder="例如：Emby JSON 库" required></div><div class="modal-field"><label>JSON 库说明</label><input name="description" value="${escC(lib?.description||'')}" placeholder="可选，填写这个库的用途"></div><div class="modal-field"><label>JSON 文件名</label><input name="path" value="${escC(edit?lib?.file?.split('/').pop()?.replace(/\.json$/i,'')||'':'')}" placeholder="例如：emby-icon" required><small class="field-help">不用填写 .json 后缀，创建时会自动添加。</small></div></div><div class="modal-actions"><button type="button" class="btn" data-action="close-modal">取消</button><button type="submit" class="btn btn-primary">${edit?'保存修改':'创建 JSON 库'}</button></div></form>`); }
