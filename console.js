...
function libraryPickerModal(anchor=document.querySelector('.library-picker')) {
 ...
 panel.innerHTML=`<div class="library-switch-title"><span>切换 JSON 库</span><button class="library-switch-delete" data-action="delete-library" data-id="${escC(S.selectedLibrary)}">删除当前库</button></div><div class="library-switch-list">${sortedLibrariesC(S.libraries).map(lib=>`<div class="library-switch-row"><button class="library-switch-option" data-action="select-library" data-id="${escC(lib.id)}" aria-checked="${lib.id===S.selectedLibrary}"><span class="library-switch-copy"><b>${escC(lib.name)}</b></span>${lib.id===S.selectedLibrary?'<em aria-label="当前库">✓</em>':''}</button></div>`).join('')}</div><button class="library-picker-create" data-action="new-library">＋ 新建 JSON 库</button>`;
