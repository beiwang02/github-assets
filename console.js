...
function libraryPickerModal() { const current=S.libraries.find(x=>x.id===S.selectedLibrary), libraries=sortedLibrariesC(S.libraries); openC(`<div class="modal-head"><div><h2>选择 JSON 库</h2><p>切换后会显示该库的图片引用。</p></div><button class="modal-close" data-action="close-modal">×</button></div><div class="modal-body library-picker-modal"><div class="library-picker-heading"><h3>已有 JSON 库</h3>${current?`<button type="button" class="text-link library-picker-delete" data-action="delete-library" data-id="${escC(current.id)}" aria-label="删除当前库 ${escC(current.name)}">删除当前库</button>`:''}</div><div class="library-picker-list">${libraries.map(lib=>lib.id===current?.id?`<div class="library-picker-row selected"><button class="library-picker-option selected" data-action="select-library" data-id="${escC(lib.id)}"><span class="library-logo" style="background:${lib.gradient||'linear-gradient(135deg,#6672ff,#7b8af1)'}">${escC(lib.name.slice(0,1))}</span><span><b>${escC(lib.name)}</b><small>${lib.count} 个图片引用 · ${escC(lib.file)}</small></span><em>当前</em></button></div>`:`<button class="library-picker-option" data-action="select-library" data-id="${escC(lib.id)}"><span class="library-logo" style="background:${lib.gradient||'linear-gradient(135deg,#6672ff,#7b8af1)'}">${escC(lib.name.slice(0,1))}</span><span><b>${escC(lib.name)}</b><small>${lib.count} 个图片引用 · ${escC(lib.file)}</small></span><i>›</i></button>`).join('')}</div><button class="library-picker-create" data-action="new-library">＋ 新建 JSON 库</button></div>`); }
