...
assert(css.includes('#tokenLoginForm .token-guide-button.ui-button,\n.json-workspace .library-switch-trigger.ui-button{justify-content:flex-start;text-align:left;padding:0;min-height:44px;border:0;background:transparent;font-size:12px}'));
assert(css.includes('.json-workspace .json-workspace-top{flex-direction:column;align-items:flex-start;gap:8px}'));
assert(css.includes('#uploadForm .modal-inline-create{justify-content:flex-start;align-items:flex-start;text-align:left;padding:0;min-height:40px;border:0;background:transparent}'));
...
assert(read('index.html').includes('ui-refresh.css?v=copy-outline-34'));
