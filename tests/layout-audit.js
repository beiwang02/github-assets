...
window.auditScoped=async()=>{
 ...
 S.auth=null;renderC();await wait();const input=document.querySelector('#mainTokenInput'),b=document.querySelector('.token-eye');input.value='fixture-not-a-token';input.focus();const cs=getComputedStyle(input),card=document.querySelector('.auth-card');check(theme+'/input-focus',cs.boxShadow==='none'&&cs.outlineStyle==='none'&&cs.borderTopWidth==='1px'&&cs.borderTopColor===(theme==='dark'?'rgb(96, 165, 250)':'rgb(37, 99, 235)'));check(theme+'/login-mobile-fit',(innerWidth>700||parseFloat(cs.fontSize)>=16)&&cs.minWidth==='0px'&&document.documentElement.scrollWidth<=innerWidth&&card.scrollWidth<=card.clientWidth);const snapshots=[];
