...
window.auditScoped=async()=>{
 ...
 S.auth=null;renderC();await wait();const input=document.querySelector('#mainTokenInput'),b=document.querySelector('.token-eye');input.value='fixture-not-a-token';input.focus();check(theme+'/input-focus',getComputedStyle(input).boxShadow==='none'&&getComputedStyle(input).outlineStyle==='none');const snapshots=[];
 ...
 check(theme+'/eye-twice',snapshots.every((s,i)=>s.type===(i===1?'text':'password')&&s.icon===(i===1?'eye-off':'eye')&&s.label===(i===1?'隐藏 Token':'显示 Token')&&s.pressed===String(i===1)&&!s.text&&s.value==='fixture-not-a-token'&&s.box===snapshots[0].box));
 }
