window.auditLibraryInfo=async()=>{
 ...
 assert(!info.closest('button,[data-action]'),'info actionable');info.click();assert(!document.querySelector('.floating-menu'),'info opened');
 ...
 document.querySelector('[data-action="edit-library"]').click();assert(!document.querySelector('.floating-menu')&&document.querySelector('#libraryForm'),'edit opens switch');closeC();
 window.copyC=async()=>{};document.querySelector('.library-outside-actions [data-action="copy"]').click();assert(!document.querySelector('.floating-menu'),'copy opens switch');
 for(const id of ['lib0','lib1']){trigger.click();await wait();const panel=document.querySelector('.floating-menu'),r=panel.getBoundingClientRect();assert(r.left>=0&&r.right<=innerWidth&&r.top>=0&&r.bottom<=innerHeight,'popup bounds');assert(!panel.querySelector('button button'),'nested button');
 ...
 trigger.click();await wait();document.querySelector('.floating-menu [data-action="new-library"]').click();assert(document.querySelector('#libraryForm'),'new missing');closeC();
 trigger.click();await wait();document.querySelector('.floating-menu [data-action="select-library"][data-id="lib1"]').click();assert(S.selectedLibrary==='lib1','switch failed');
 S.view='overview';renderC();const row=document.querySelector('.library-empty-row');assert(row&&row.dataset.action==='open-library','overview entry');row.click();assert(S.view==='libraries'&&!document.querySelector('.floating-menu'),'overview popup');
